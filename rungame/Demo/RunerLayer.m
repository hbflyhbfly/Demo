...
-(BOOL)isCollision:(CCSprite*)coll{
    BOOL bRet = NO;
    CGRect runerRect = __runner.boundingBox;
    CGPoint collLocation = coll.position;
    CGPoint location = __runner.position;
    CGRect collRect = coll.boundingBox;
    COLL_AREA collArea;
    //left
    if (CGRectContainsPoint(collRect, ccpAdd(location, ccp(0, runerRect.size.height/2)))) {
        collArea = COLL_STATE_LEFT;
        //location = ccp(collL, <#CGFloat y#>)
        bRet = YES;
        NSLog(@"left");
    }
    //right
    else if (CGRectContainsPoint(collRect, ccpAdd(location, ccp(runerRect.size.width, runerRect.size.height/2)))) {
        collArea = COLL_STATE_RIGHT;
        location = ccp(collLocation.x-runerRect.size.width, location.y);
        bRet = YES;
        NSLog(@"right");
    }
    //up
    else if (CGRectContainsPoint(collRect, ccpAdd(location, ccp(runerRect.size.width/2, runerRect.size.height)))) {
    ...
    //bottom
        ...
        if (CGRectContainsPoint(collRect, ccpAdd(location, ccp(runerRect.size.width/2, 0)))) {
